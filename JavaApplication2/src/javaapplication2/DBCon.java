...
public class DBCon {
     public static Connection JavaCon() {
        try {
            ...
            Connection conn = DriverManager.getConnection(url,"sa", "wkx971220");
            return conn;
    ...
    public static Vector prepareQueryData(String BookID, String BookName,String Status, String Press,String PressDate,String Author) {
        //调用纯Java方式连接数据库
        Connection conn = JavaCon( );
        //声明保存查询结果的Vector对象
        Vector data = new Vector( );
        try {
            ...
            PreparedStatement pstmt = conn.prepareStatement(
                        "Select * from Book where BookID=? and BookName=? and Status=? and Press=? and PressDate=? and Author=?");
            //设置预编译参数
    ...
    public static boolean prepareUpdateData(String BookID, String BookName,String Status, String Press,String PressDate,String Author) {
        //调用纯Java方式连接数据库
        Connection conn = JavaCon();
        try {
            //在连接对象的基础上创建预编译会话对象
